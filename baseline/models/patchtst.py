...
from utils.layers import Transpose, get_act_fn, RevIN
...
class _PatchTST_backbone(nn.Module):
    def __init__(self, c_in, seq_len, pred_dim, patch_len, stride, 
                 ...
                 revin=True, affine=True, subtract_last=False, num_classes=45):
        ...
        self.patch_len = patch_len
        self.stride = stride
        # 使用整除直接截断尾部不足一个patch的部分
        self.patch_num = (seq_len - patch_len) // stride + 1
        
        # 移除padding层，改为直接截断
        self.padding_patch_layer = nn.Identity()  # 不再需要填充
        
        # 添加安全验证
        if self.patch_num < 1:
            raise ValueError(f"Invalid parameters: seq_len({seq_len}) < patch_len({patch_len})")
        ...
        # Backbone 
        self.backbone = _TSTiEncoder(c_in, patch_num=self.patch_num, patch_len=patch_len,
        ...
        if individual:
            ...
            for i in range(self.n_vars):
                self.layers.append(nn.Sequential(
                    ...
                    nn.Linear(self.head_nf, num_classes),  # 输出类别数
                    nn.Softmax(dim=-1)
                ))
        else:
    ...
    def forward(self, z:Tensor): 
        ...
        # 验证输入长度足够
        if s < self.patch_len:
            raise ValueError(f"Input length {s} < patch_len {self.patch_len}")
            
        # 计算实际可用长度
        valid_len = (self.patch_num - 1) * self.stride + self.patch_len
        z = z[..., :valid_len]  # 截断多余部分
        
        # 展开操作
        z = z.reshape(-1, 1, 1, valid_len)
        z = self.unfold(z)
...
class PatchTST(nn.Module):
    def __init__(self,
         ...
         stride=8,  # stride
         padding_patch=False,  # flag to indicate if padded is added if necessary
         revin=True,  # RevIN
        ...
        if self.decomposition:
            ...
            self.model = _PatchTST_backbone(c_in=c_in, seq_len=seq_len, pred_dim=pred_dim, 
                                            ...
                                            dropout=dropout, act=activation, res_attention=res_attention, pre_norm=pre_norm, 
                                            store_attn=store_attn, padding_patch=padding_patch, 
                                            individual=individual, revin=revin, affine=affine, subtract_last=subtract_last, num_classes=45)
